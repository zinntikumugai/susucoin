...
#if defined(NDEBUG)
# error "Susucoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Susucoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s Susu additional fees, %d delta bytes\n",
...
{
    LOCK(cs_main);
    // Enable witness over block 4885
    if (pindexPrev->nHeight >= 482) {
      return true;}
