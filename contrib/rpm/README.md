...
The RPM spec file provided here is for Susucoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://susucoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Susucoin git repository](https://github.com/susucoin/susucoin) but are not part of
...
Currently only `Source10` is used, for the example `susucoin.conf` file.
...
in the susucoin git repository. At present only an SVG version of the Susucoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the susucoin
developers so that an official fix to the issue can make it into the next
release.

### Patch0 susucoin-0.12.0-libressl.patch
...
needed when 0.12.1 is released, a proper fix is already in the Susucoin git
master branch.
...
idea to build Susucoin Core against a static version of that library compiled
...
The `susucoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `susucoin-server` package. At this point, most RPM based
...
The `susucoin-server` package should have SELinux support. How to properly do
...
Susucoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/susucoin/). He has been
packaging Susucoin for Fedora at least since 2012.
...
4. I renamed the `susucoin` package that contains the Qt GUI to `susucoin-core` as
that appears to be how the general population refers to it, in contrast to
`susucoin-xt` or `susucoin-classic`. I wanted to make sure the general population
