...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8876
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8777
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18876
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18877
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18843
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18844
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 8876
	%{_sbindir}/semanage port -d -p tcp 8777
	%{_sbindir}/semanage port -d -p tcp 18876
	%{_sbindir}/semanage port -d -p tcp 18877
	%{_sbindir}/semanage port -d -p tcp 18843
	%{_sbindir}/semanage port -d -p tcp 18844
	for selinuxvariant in %{selinux_variants}; do
